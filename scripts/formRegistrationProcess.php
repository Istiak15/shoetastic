...
if ($origin != "/~u17/submissions/submission04/pages/formRegistration.php") {
...
if (emailAlreadyExists($db, $_POST['email'])) {
    ...
    if ($uniqueLoginName != $_POST['loginName']) {
        echo "<h3>Your preferred login name already exists. So we have assigned \"$uniqueLoginName\" as your login name.</h3>";
    }
    $firstName = str_replace("'", "\'", $firstName);
    $lastName = str_replace("'", "\'", $lastName);

    $query = "INSERT INTO my_Customer
              (salutation, first_name, middle_initial, last_name, gender, email, phone, street, city, region, postal_code, date_time, login_name, login_password)
              VALUES
              ('$salutation', '$firstName', '$middleInitial', '$lastName',
               '$gender', '$email', '$phone', 
    ...
    if (mysqli_query($db, $query)) {
        echo "<h3>Thank you for registering with Amazing Stuff.<br>
              ...
              To log in and start shopping in our e-store, please <a href='pages/formLogin.php'>click here</a>.</h3>";
    } else {
